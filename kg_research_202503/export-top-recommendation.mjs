...
const summaryPath = path.join(repoRoot, "research", "kg_research_202503", "output", "summary.json");
...
function resolveNodeLayout(graph, layoutData) {
  const width = 2240;
  const height = 1360;
  const bandY = { lv1: 220, lv2: 670, lv3: 1100 };
  ...
  const nodes = graph.nodes.map((node) => {
    const style = nodeStyle(node.level);
  ...
  for (let iteration = 0; iteration < 320; iteration += 1) {
    for (let left = 0; left < nodes.length; left += 1) {
      for (let right = left + 1; right < nodes.length; right += 1) {
        const a = nodes[left];
        const b = nodes[right];
        const requiredX = (a.width + b.width) / 2 + 70;
        const requiredY = (a.height + b.height) / 2 + 34;
        const dx = b.x - a.x;
        const dy = b.y - a.y;
  ...
  for (const level of ["lv1", "lv2", "lv3"]) {
    ...
    for (let index = levelNodes.length - 2; index >= 0; index -= 1) {
      ...
      const requiredGap = (current.width + next.width) / 2 + gapBonus;
  ...
  const positioned = Object.fromEntries(nodes.map((node) => [node.id, node]));
  return { width, height, positioned, nodes };
}
...
function renderPreviewHtml(artifact, outputRelativeJsonPath) {
  const { document, graph, activeEdges, score, strategy, provider, layout, edgePolicy } = artifact;
  const resolved = resolveNodeLayout(graph, artifact.layoutData);

  const edgeSvg = activeEdges
    .map((edge) => {
...
<head>
  <meta charset="utf-8" />
  <title>${escapeHtml(document.title)} - cleaned review preview</title>
  <style>
...
<body>
  <div class="shell">
    ...
    <div class="meta">
      ...
      <div class="card"><strong>Visible edges</strong><p style="margin-top:6px">${activeEdges.length}</p></div>
...
<head>
  <meta charset="utf-8" />
  <title>GraphMind Top Recommendation Visual Review</title>
  <style>
    :root { color-scheme: dark; }
    body {
      margin: 0;
    ...
    .meta p { margin: 4px 0 0; color: rgba(226,232,240,0.68); }
    .score {
      white-space: nowrap;
      padding: 8px 12px;
      border-radius: 999px;
...
<body>
  <main class="shell">
    <section class="hero">
      <h1>Top Recommendation Visual Review</h1>
      <p>Bundled previews for the current winning GraphMind research configuration.</p>
      <div class="pill-row">
...
async function main() {
  ...
  await ensureDir(recommendedDir);
  ...
  for (const result of matchingResults) {
    const sourceJson = path.join(repoRoot, normalizeArchivedResultPath(result.jsonPath));
    const artifact = JSON.parse(await fs.readFile(sourceJson, "utf8"));
    const documentSlug = result.documentId;
    const htmlFile = `${documentSlug}.html`;
    const jsonFile = `${documentSlug}.json`;
    await copyFile(sourceJson, path.join(recommendedDir, jsonFile));
    await fs.writeFile(
      path.join(recommendedDir, htmlFile),
      renderPreviewHtml(artifact, jsonFile),
      "utf8"
    );
  ...
  await fs.writeFile(
    path.join(recommendedDir, "index.html"),
    buildIndexHtml(bundle),
    "utf8"
  );
  await fs.writeFile(
    path.join(recommendedDir, "manifest.json"),
  ...
  console.log(
    JSON.stringify(
      {
        winner,
        outputDir: path.relative(repoRoot, recommendedDir),
